                ...
                if '/' in voc:
                    ...
                    for voc_temp in voc_tempList:
                        if voc_temp != '':
                            ...
                            if voc_temp == -1:
                                    continue
                            singleLevelData.append([voc_temp, level_index])
                elif '(' in voc:
                    voc_tempList = voc.split('(')
                    ...
                    if voc_temp == -1:
                                continue
                    singleLevelData.append([voc_temp, level_index])
                    voc_temp = removeSpaceInTheEnd(voc_tempList[1])
                    if voc_temp == -1:
                                continue
                    singleLevelData.append([(voc_temp + voc_tempList[1]), level_index])
                else:
...
def preprocess_word(levelVocData): 
    ...
    for levelData in levelVocData:
        for vocData in levelData:
            ...
            target = vocData[1] - 1 # since a var y in range 0 - 9 will become a vector whose length is 10 with keras.utils.to_categorical 
    ...
    return [charNumList, vocCharCombinationCountList, targetList]

    
def getDataset(charNumList, vocCharCombinationCountList, targetList):
    ...
    for i in range(len(charNumList)):
        if i%10 in train_index:
            x_train.append(charNumList[i] + vocCharCombinationCountList[i])
            y_train.append(targetList[i])        
        elif i%10 == test_index:
            x_test.append(charNumList[i] + vocCharCombinationCountList[i])
...
def main():
    ...
    [charNumList, vocCharCombinationCountList, targetList] = preprocess_word(levelVocData)

    x_train, y_train, x_test, y_test = getDataset(charNumList, vocCharCombinationCountList, targetList)
